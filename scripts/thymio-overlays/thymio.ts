...
async function connect() {
  if (device && device.gatt) {
    ...
    // Resolve other characteristics but do not start their notifications yet.
    // Extra notify traffic shares the BLE link with motor write+ACK and was
    // starving closed-loop control (~3–4 Hz). Enable them lazily when used.
    ...
    console.log(
      "Command write mode:",
      command.getCommandWriteMode(commandCharacteristic),
...
/**
 * Fast motor-only update (single packet, preferably without response).
 */
export async function setMotorSpeeds(motorLeft: number, motorRight: number) {
  return await command.setMotorSpeeds(commandCharacteristic, motorLeft, motorRight);
}

/** "without-response" | "with-response" | "unknown" — explains motor Hz limits. */
export function getMotorWriteMode() {
  return command.getCommandWriteMode(commandCharacteristic);
}

// PYTHON CHARACTERISTIC

/**
 * Start Python + stdout notifications (deferred at connect to keep the BLE
 * link free for sensors). Call before sendPythonScript / executeLoadedScript.
 */
export async function ensurePythonChannel() {
  if (!pythonCharacteristic || !stdOutCharacteristic) {
    throw new Error("Bluetooth GATT is not connected");
  }
  if (pythonChannelReady) return;
